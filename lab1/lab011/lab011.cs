...
class lab011 {
    static void Main() {
        // В данном задании нам нужно получить константы(минимальное, максимальное значения) у каждого типа
        // "Под капотом" каждый тип данных (int, float, string) является классом
        // поэтому и у них существуют поля, методы, к которым можно обратиться
